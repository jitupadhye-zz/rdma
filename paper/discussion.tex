...
\end{thm}
\begin{proof}
To guarantee fairness, a necessary condition is that the system have a
unique fixed point.

Suppose there are $N$ flows sharing a link of capacity $C$. 
Then, for each flow, we have 
$ R_i = f(d,p_i), i = 1,\ldots,N.$
There is an additional equation constraining the throughput at the
link, $\sum R_i = C$.
Hence we have $N+1$ equations and $2 N$ unknowns $\{R_i,p_i\},$ with $ i=1\ldots,N,$ giving
rise to an underdetermined system with infinite solutions (or no
solution). To make this system consistent, we need a common $p_i$,
