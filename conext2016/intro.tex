...
point. \fixme{$(iii)$ We compare the flow completion time of TIMELY and 
DCQCN running the same traffic traces. DCQCN outperforms TIMELY
due to better fairness and stability. Patched TIMELY closes the gap, but 
still cannot match DCQCN performance. We shed light on this result with the discussion below.}.
...
delay-based congestion control. \fixme{Finally, ECN signal is more resilient to 
jitters on the backward path.}
