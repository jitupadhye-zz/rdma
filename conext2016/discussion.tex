...
The fundamental reasons that ECN-based protocols may be better than delay-based 
protocols are then discussed. 
...
To further confirm that ECN marking on egress is important for DCQCN stability,
...
\end{proof}
\para{Remarks:} Note that a unique fixed point can be guaranteed by
having a \textbf{RED} style AQM scheme that has a fixed point that
depends on the number of flows. However, given the low delay
...
controller creating a separate tradeoff. 
...
can delay the arrival of ECN signal, or interfere round-trip delay measurement, thus affecting
the protocol performance and stability.
Although DCQCN and TIMELY both attempt to mitigate this artifact, {\em e.g.,} by prioritizing
feedback packets, the feedback transmission cannot be jitter-free as the hardware jitter or 
congestion on backward path is unavoidable in certain cases. 

Intuitively, ECN marking is more
