...
\vspace{-0.5em}
\para{Remarks:} 
Note that a unique fixed point can be guaranteed by
...
with which rate-based protocols do not have steady throughput.
...
\vspace{-1em}
\caption{DCQCN is more resilient to variable feedback transmission delay than TIMELY.}
\vspace{-1em}
...
and TIMELY ($\tau '$) models. When we add jitter in the $[0,100]$
range to TIMELY, it becomes extremely unstable compared to the same
scenario without the additional jitter shown in
Figure~\ref{fig:timely_fixed_a}. In contrast, the same level of
additional jitter makes no difference to DCQCN, only when the jitter
increases to the $[0,200]$ range does DCQCN exhibit similar
