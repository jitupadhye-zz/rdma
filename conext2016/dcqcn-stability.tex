...
\fixme{We point out that $p^*$ uniquely determines the fixed queue length $q^*$ and depends on 
the number of flows $N$. This is not ideal, since we hope the queue length 
stays the same regardless of the number of flows. With ECN as the signal, we may achieve this
