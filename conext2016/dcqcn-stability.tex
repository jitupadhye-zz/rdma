...
\subfigure[$R_{AI}=10Mbps$, $K_{max}=1000KB$.]
{
...
In \S~\ref{sec:dcqcn_convergence}, we prove that \note{flow rates do not reach a steady state 
until} they converge to the same rate, {\em i.e.,}
\begin{equation}
\small
R_C^{(1)*} = R_C^{(2)*} = ... = R_C^{(N)*}
\end{equation}
 
\note{The fixed point, where by definition the flow rates are steady, must satisfy this equation.} 
...
\note{A stable system must have negative {\em Gain} (in dB) when there is a small 
oscillation aruond the fixed point, so that it converges back to the fixed point. 
{\em Phase Margin} is defined as how far the system
is from the 0dB {\em Gain} state. The system is stable when its {\em Phase Margin} 
is larger than 0, and the larger {\em Phase Margin} means the system is more stable.
{\em Phase Margin} is computed from the characteristic equation of a system. See 
Appendix~\ref{sec:dcqcn_stability_analysis} for details of the derivation of the
characteristic equation for $R_C$.}
...
control signal delays $\tau *$, and different number of flows. 
An ideal protocol should be tolerant with control signal delays
and scalable to any number of flows. 
\note{In practice, $\tau *$ is dominated by propagation delay since ECN marking is not 
affected by the queuing delay at the congestion point (see Section~\ref{sec:discuss}). In 
data center environment, propagation delay is usually well below 100$\mu$s. There could be
tens of flows competing for a single bottleneck.}
As Figure~\ref{fig:dcqcn_stability} shows, DCQCN, with default parameters, is mostly stable
in such environment. 
...
\if 0
\para{Stability at 100Gbps.}  As the data center network fabric is moving towards
